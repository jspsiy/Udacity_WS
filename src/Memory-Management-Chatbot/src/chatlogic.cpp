...
ChatLogic::~ChatLogic()
{
    ...
    //chatbot is not created via unique pointer and is needed to be a permanent element while the system is running do not delete
    //delete _chatBot;

    // delete all nodes //these are smart pointers these codes are not needed.
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                            std::unique_ptr<GraphEdge> edge(new GraphEdge(id));//std::make_unique<GraphEdge>(id);
   ...
   ChatBot chatter("../images/chatbot.png");
    chatter.SetChatLogicHandle(this);
    chatter.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(chatter));
